...
void CLogWindow::AddMessage(int Type, LPCSTR Message)
{
	...
	m_View.SetItemData(id,(DWORD)Type);
	m_View.Scroll(size);
}
...
{
	// TODO : ここにコマンド ハンドラ コードを追加します。
	CFileDialog dlg(FALSE,".txt",NULL,OFN_HIDEREADONLY |OFN_OVERWRITEPROMPT,STR(1));
...
void CLogWindow::SetLanguage(void)
{
	...
	for(i=0;i<3;i++){
		GetMenu()->ModifyMenu(MenuString[i][0],MF_BYCOMMAND | MF_STRING,MenuString[i][0],theSetting.m_Lang.m_Str[LP_LOGMENU + MenuString[i][1]]);
	}
	SetWindowText(STR(0));
}
