...
bool CCDWriter::ParseCueSheetFile(LPCSTR FileName)
{
	...
	*p = '\0';
	...
	ReadFile(hFile,p,FileSize,&read,NULL);
	p[FileSize] = '\0';
	CloseHandle(hFile);
...
bool CCDWriter::StartWriting(bool ModeMS)
{
	...
	if(ModeMS){
		m_MMCWriter.PreventMediaRemoval(1);
	} else {
...
void CCDWriter::FinishWriting(void)
{
	...
	m_MMCWriter.PreventMediaRemoval(0);
	m_MMCWriter.ReWind();
}
...
bool CCDWriter::WriteRawLeadIn(void)
{
	...
	lba = (int)m_SubCode.GenerateSub16(m_WriteBuffer + 2352);
...
//		m_EDC.Mode1Raw((LPSTR)Buffer,(BYTE)(lba / (75 * 60)),(BYTE)((lba / 75) % 60),(BYTE)(lba % 75));
		if(m_DataMode == 1){
			...
			Buffer[12] = m_CueSheetParser.ToHex((BYTE)(lba / (75 * 60)));
			Buffer[13] = m_CueSheetParser.ToHex((BYTE)((lba / 75) % 60));
			Buffer[14] = m_CueSheetParser.ToHex((BYTE)(lba % 75));
			Buffer[15] = 2;
			Buffer[0x12] = 0x20;
			Buffer[0x16] = 0x20;
		}
