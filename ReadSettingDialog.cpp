...
// CReadSettingDialog メッセージ ハンドラ

void CReadSettingDialog::OnBnClickedBrowsfile()
{
	// TODO : ここにコントロール通知ハンドラ コードを追加します。
	if(m_EngineList.GetCurSel() == 3){
		CFileDialog dlg(TRUE,NULL,NULL,OFN_HIDEREADONLY |OFN_OVERWRITEPROMPT,MSG(22));
		...
		CFileDialog dlg(FALSE,NULL,NULL,OFN_HIDEREADONLY |OFN_OVERWRITEPROMPT,MSG(22));
...
void CReadSettingDialog::OnBnClickedOk()
{
...

	{
		...
		if((theSetting.m_ReadEngine == 1 ||theSetting.m_ReadEngine == 2)&& !theSetting.m_TestReadMode && m_ImageName.Right(3).MakeLower() == "cue"){
			m_ImageName.Format("%s.cdm",Buffer);
		}
		if(!(theSetting.m_ReadEngine == 1 ||theSetting.m_ReadEngine == 2) && !theSetting.m_TestReadMode && m_ImageName.Right(3).MakeLower() != "cue"){
			m_ImageName.Format("%s.cue",Buffer);
