...
bool CMMCReader::ReadTOCFromSession(TableOfContents & Toc)
{
	...
	//   Execute ReadTOC command
	{
		...
		cmd.SRB_BufPointer = (BYTE FAR *)(Toc.m_RawTOC);
		cmd.SRB_CDBLen = 10;
		cmd.CDBByte[0]		= 0x43;
		cmd.CDBByte[1]		= 0x02;
		cmd.CDBByte[2]		= 0x02;
		cmd.CDBByte[7]		= (BYTE)((sizeof(Toc.m_RawTOC) / 0x100) & 0xff);
		cmd.CDBByte[8]		= (BYTE)(sizeof(Toc.m_RawTOC) & 0xff);
	...
	//   Modify result data
	{
		...
		for(i=0;i<InfoNum;i++){
			...
			if((TrackData[1] & 0xf0) == 0x10 && (TrackData[3] < 100)){	//   session info
				...
				if(TrackData[0] > 1){	//   もしセッション２以降なら、前セッションの最後を指定
...
bool CMMCReader::ReadSubQ(MSFAddress msf, BYTE * Buffer)
{
	...
	cmd.SRB_BufPointer = (BYTE*)ReadBuffer;
	cmd.SRB_CDBLen = 12;
	cmd.SRB_Flags      = SRB_DIR_IN;
	cmd.CDBByte[ 0] = 0xBE;		//   READ CD LBA
	cmd.CDBByte[ 2] = (BYTE)(lba >> 24);
...
int CMMCReader::ReadCDText(BYTE * Buffer)
{
	...
	cmd.CDBByte[7]		= (BYTE)((5000 / 0x100) & 0xff);
	cmd.CDBByte[8]		= (BYTE)(5000 & 0xff);
