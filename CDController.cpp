...
bool CCDController::ReadCDAudio(MSFAddress MSF, BYTE * Buffer)
{
	...
	} else if(theSetting.m_ReadAudioMethod == 1){
		if(m_Reader.ReadCDDA_LBA(MSF,(LPSTR)Buffer)){
			return true;
		}
	} else if(theSetting.m_ReadAudioMethod == 2){
		if(m_Reader.ReadCDDA(MSF,(LPSTR)Buffer)){
			return true;
		}
	} else if(theSetting.m_ReadAudioMethod == 3){
		if(m_Reader.ReadCD_LBA(MSF,(LPSTR)Buffer)){
			return true;
		}
	} else if(theSetting.m_ReadAudioMethod == 4){
		if(m_Reader.ReadCD(MSF,(LPSTR)Buffer)){
			return true;
		}
	} else if(theSetting.m_ReadAudioMethod == 5){
		if(m_Reader.ReadCDRaw_LBA(MSF,(LPSTR)Buffer)){
			return true;
		}
	} else if(theSetting.m_ReadAudioMethod == 6){
		if(m_Reader.ReadCDRaw(MSF,(LPSTR)Buffer)){
			return true;
		}
	} else if(theSetting.m_ReadAudioMethod == 7){
		if(m_Reader.ReadCD_Read10(MSF,(LPSTR)Buffer)){
			return true;
		}
	} else if(theSetting.m_ReadAudioMethod == 8){
		if(m_Reader.ReadCD_D4(MSF,(LPSTR)Buffer)){
			return true;
		}
	} else if(theSetting.m_ReadAudioMethod == 9){
		if(m_Reader.ReadCD_D4_2(MSF,(LPSTR)Buffer)){
			return true;
		}
	} else if(theSetting.m_ReadAudioMethod == 10){
