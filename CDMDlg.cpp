...
BOOL CAboutDlg::OnInitDialog()
{
	...
	m_VerString.Format("CD Manipulator Version %s",APP_VERSION);

	SetWindowText(theSetting.m_Lang.m_Str[LP_ABOUT + 0]);
	this->SetDlgItemText(IDC_SUBTITLE,theSetting.m_Lang.m_Str[LP_ABOUT + 1]);
	this->SetDlgItemText(IDC_AUTHOR,"Copyright (C) 2002-2004 Y.Kanechika（ごーるど）");
...
BOOL CCDMDlg::OnInitDialog()
{
	...
	if(theSetting.m_UseSPTI){
		...
		if(m_CD.GetAspiCtrl()->IsActive()){
			...
			cs.Format("ASPI version:%d.%d.%d.%d",Version & 0xff,(Version >> 8) & 0xff,(Version >> 16) & 0xff,Version >> 24);
...
//		CTime tm = CTime::GetCurrentTime();
		...
		m_LogWnd->LoadFrame(IDR_LOGFRAME,WS_OVERLAPPEDWINDOW | FWS_ADDTOTITLE,NULL,NULL);

		//   set start log
		cs.Format("CD Manipulator Version %s",APP_VERSION);
		m_LogWnd->AddMessage(LOG_INFO,cs);
	...
	if(m_TocWnd == NULL){
		...
		m_TocWnd->LoadFrame(IDR_TOCFRAME,WS_OVERLAPPEDWINDOW | FWS_ADDTOTITLE,NULL,NULL);
		//
	} else {
		m_TocWnd->ClearList();
	}
...
void CCDMDlg::OnLanguage()
{
	...
	if(Dlg.DoModal() == IDOK){
		theSetting.m_Lang.ReadLanguage(theSetting.m_LangFile);
...
void CCDMDlg::InitializeButtons(void)
{
	...
	if(theSetting.m_SkinFile != ""){
		...
		bmp.m_hObject = LoadImage(NULL,theSetting.m_SkinFile,IMAGE_BITMAP,0,0,LR_LOADFROMFILE);
		if(bmp.m_hObject != NULL){
			...
			m_CreateImageBmp.CreateCompatibleBitmap(pDC,101,45);
			m_WriteImageBmp.CreateCompatibleBitmap(pDC,101,45);
			m_DuplicateBmp.CreateCompatibleBitmap(pDC,101,45);
			m_ReadTrackBmp.CreateCompatibleBitmap(pDC,101,45);
			m_MasteringBmp.CreateCompatibleBitmap(pDC,101,45);
			m_SubQBmp.CreateCompatibleBitmap(pDC,101,45);
			m_RecognizeBmp.CreateCompatibleBitmap(pDC,101,45);
			m_ExitBmp.CreateCompatibleBitmap(pDC,101,45);

			BmpDC.SelectObject(&m_CreateImageBmp);
			BmpDC.BitBlt(0,0,101,45,&SkinDC,0,0,SRCCOPY);
			BmpDC.SelectObject(&m_WriteImageBmp);
			BmpDC.BitBlt(0,0,101,45,&SkinDC,101,0,SRCCOPY);
			BmpDC.SelectObject(&m_DuplicateBmp);
			BmpDC.BitBlt(0,0,101,45,&SkinDC,202,0,SRCCOPY);
			BmpDC.SelectObject(&m_RecognizeBmp);
			BmpDC.BitBlt(0,0,101,45,&SkinDC,303,0,SRCCOPY);
			BmpDC.SelectObject(&m_ReadTrackBmp);
			BmpDC.BitBlt(0,0,101,45,&SkinDC,0,45,SRCCOPY);
			BmpDC.SelectObject(&m_MasteringBmp);
			BmpDC.BitBlt(0,0,101,45,&SkinDC,101,45,SRCCOPY);
			BmpDC.SelectObject(&m_SubQBmp);
			BmpDC.BitBlt(0,0,101,45,&SkinDC,202,45,SRCCOPY);
			BmpDC.SelectObject(&m_ExitBmp);
			BmpDC.BitBlt(0,0,101,45,&SkinDC,303,45,SRCCOPY);
	...
	m_CreateImageButton.SetBitmap((HBITMAP)m_CreateImageBmp.m_hObject);
	m_WriteImageButton.SetBitmap((HBITMAP)m_WriteImageBmp.m_hObject);
	m_DuplicateButton.SetBitmap((HBITMAP)m_DuplicateBmp.m_hObject);
	m_ReadTrackButton.SetBitmap((HBITMAP)m_ReadTrackBmp.m_hObject);
	m_MasteringButton.SetBitmap((HBITMAP)m_MasteringBmp.m_hObject);
	m_SubQButton.SetBitmap((HBITMAP)m_SubQBmp.m_hObject);
	m_RecognizeButton.SetBitmap((HBITMAP)m_RecognizeBmp.m_hObject);
	m_ExitButton.SetBitmap((HBITMAP)m_ExitBmp.m_hObject);
}
...
{
	{
		...
		if(theSetting.m_Lang.m_Str[LP_MAINMENU + i][0] == '\0'){
			GetMenu()->ModifyMenu(0,MF_BYPOSITION | MF_STRING,0,"CD Manipulator");
		}
		for(i=0;i<17;i++){
			GetMenu()->ModifyMenu(MenuString[i][0],MF_BYCOMMAND | MF_STRING,MenuString[i][0],theSetting.m_Lang.m_Str[LP_MAINMENU + MenuString[i][1]]);
		}
...
void CCDMDlg::OnHelpHelp()
{
	// TODO : ここにコマンド ハンドラ コードを追加します。
	CString cs;
	cs.Format("%sCdManipulator.chm",theSetting.m_Dir);
