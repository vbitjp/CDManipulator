// Aspi.cpp: CAspiDriver クラスのインプリメンテーション
//
//////////////////////////////////////////////////////////////////////
...
void CAspiDriver::Initialize()
{
	...
	if(m_hModule == NULL){
		MessageBox(NULL,MSG(0),ERROR_MSG,MB_OK);
		return;
	}
...
{
	if(m_hModule == NULL || m_GetASPI32DLLVersion == NULL){
		return 0;
	} else {
...
void CAspiDriver::ExecuteCommand(SRB_ExecSCSICmd &cmd)
{
	...
	cmd.SRB_PostProc = (LPVOID)m_hEvent;		// Set notify event
...
void CAspiDriver::ScanBus(void)
{
	...
	for(Ha = 0;Ha < HaMax; Ha++){
		...
		ha_cmd.SRB_Ha = Ha;
		m_SendASPI32Command((LPSRB)&ha_cmd);
		...
		for(Tgt = 0; Tgt < TgtMax; Tgt++){
			...
			m_SendASPI32Command((LPSRB)&dev_cmd);
