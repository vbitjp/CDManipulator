...
bool CSubcodeGeneratorMS::ParseFile(LPCSTR FileName)
{
	...
	if(i > 1){
		...
		i = GetPrivateProfileInt("CDManipulator","Version",0,FileName);
		if(i > 0){
			char Buf[100];
			m_CDM_Extension = true;
			m_ImageVersion.Format("CD Manipulator Format Ver.%d",i);
	...
	for(i=0;i<m_TocIntCount;i++){
		...
		if((toc[1] & 0x0f) == 5){
			if(toc[3] <= 40){
				...
				toc[4] = 0x60;
				toc[8] = (BYTE)(m_LeadIn / (75*60));
				toc[9] = (BYTE)((m_LeadIn / 75) % 60);
				toc[10] = (BYTE)(m_LeadIn % 75);
				for(j=8;j<11;j++){
					toc[j] = TO_HEX(toc[j]);
				}
			}
...
void CSubcodeGeneratorMS::ResetGenerator(DWORD StartLBA, int SubcodeType, int SessionNo)
{
	...
	if(SubcodeType == 0){
		...
		for(i=0;i<m_EntryCount;i++){
			toc = m_Entry + i*11;
			if(toc[0] == (BYTE)(SessionNo+1) && ((toc[1] & 0x0f) == 1) && toc[3] <= 0x63){
				if(m_FirstTrack == 0){
					m_FirstTrack = toc[3];
				}
				memcpy(p,toc,11);
		...
		for(i=0;i<m_EntryCount;i++){
			toc = m_Entry + i*11;
			if(toc[0] == (BYTE)(SessionNo+1) && ((toc[1] & 0x0f) == 0x05)){
				memcpy(q,toc,11);
				q+=11;
				m_TocIntCount++;
			} else if(toc[0] == (BYTE)(SessionNo+1) && toc[3] > 0x63){
				memcpy(p,toc,11);
				p+=11;
				m_TocCount++;
			}
...
BYTE * CSubcodeGeneratorMS::GenerateLeadIn(void)
{
	...
	if((toc[3] >= 0x01 && toc[3] <= 0x99) || (toc[3] >= 0xa0 && toc[3] <= 0xa2)){
		...
		subQ[3] = (BYTE)TO_HEX(lba / (75*60));
		subQ[4] = (BYTE)TO_HEX((lba / 75) % 60);
		subQ[5] = (BYTE)TO_HEX(lba % 75);
	}
	...
	if(m_CDTextEntries > 0){
		...
		r = m_Sub96+24;
		for(j=0;j<96;j+=4){
			EncodeBuffer[j+0] = (BYTE)((r[0] >> 2) & 0x3f);
			EncodeBuffer[j+1] = (BYTE)(((r[0] << 4) | (r[1] >> 4)) & 0x3f);
			EncodeBuffer[j+2] = (BYTE)(((r[1] << 2) | (r[2] >> 6)) & 0x3f);
			EncodeBuffer[j+3] = (BYTE)(r[2] & 0x3f);
			r += 3;
		}
		{
			BYTE *s,*t;
...
BYTE * CSubcodeGeneratorMS::GeneratePreGap(void)
{
	...
	subQ[3] = (BYTE)TO_HEX(m_RelativeLBA / (75*60));
	subQ[4] = (BYTE)TO_HEX((m_RelativeLBA / 75) % 60);
	subQ[5] = (BYTE)TO_HEX(m_RelativeLBA % 75);

	subQ[7] = (BYTE)TO_HEX(m_CurrentLBA / (75*60));
	subQ[8] = (BYTE)TO_HEX((m_CurrentLBA / 75) % 60);
	subQ[9] = (BYTE)TO_HEX(m_CurrentLBA % 75);
	...
	if(m_PreGapMode[m_CurrentSession] == 0){
		...
		if(lba > 0x80000000){
			...
			if(m_PreGapMode[m_CurrentSession] == 1){
				m_EDC.Mode1Raw(Buffer,(BYTE)(lba / (75 * 60)),(BYTE)((lba / 75) % 60),(BYTE)(lba % 75));
			} else {
