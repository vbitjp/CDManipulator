...
protected:
	BYTE m_PrevTr;
	BYTE m_PrevIdx;
	void EntryToc(BYTE Track, BYTE Idx, BYTE Minute, BYTE Second, BYTE Frame, BYTE TrackType, BYTE CtlAdr, BYTE SessionNo);
	bool m_Pregaped[99];
	...
	bool ReadSession(MSFAddress Start, MSFAddress End,int SessionNo,BYTE &LeadInMode,BYTE ISRC[100][12],int ReadingMethod);
