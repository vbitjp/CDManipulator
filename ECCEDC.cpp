...
extern const char *efm_table[256];

#define EFM(a,b) *(m_EFMTable+a*0x10+b)
...
int CCheckSector::CheckMode1Raw(LPCSTR raw, int M, int S, int F)
{
	BYTE rM, rS, rF;
	Mode1Frame *frame;
	...
	if(MemCmp(raw,(LPCSTR)EDC_Sync,12)){
		return SECTOR_BADSYNC;
	}

	if(frame->Min != rM || frame->Sec != rS || frame->Frame != rF){
		m_OriginalM = ((frame->Min / 16) * 10) + (frame->Min % 16);
		m_OriginalS = ((frame->Sec / 16) * 10) + (frame->Sec % 16);
		m_OriginalF = ((frame->Frame / 16) * 10) + (frame->Frame % 16);
		return SECTOR_BADMSF;
	}

	m_CalcEDC = CalcEDC(raw);
	if(frame->Edc != m_CalcEDC){
		m_OriginalEDC = frame->Edc;
		return SECTOR_BADEDC;
	}

	CalcEccP((BYTE*)raw+12);
	if(MemCmp(raw+2076,(char*)m_EccP,172)){
		return SECTOR_BADECC;
	}

	CalcEccQ((BYTE*)raw+12);
	if(MemCmp(raw+2248,(char*)m_EccQ,104)){
		return SECTOR_BADECC;
	}
...
int CCheckSector::MemCmp(LPCSTR Mem1, LPCSTR Mem2, DWORD Count)
{
	DWORD i;
...
DWORD CCheckSector::CalcEDC(LPCSTR Buffer)
{
	LPCSTR p = Buffer;
	DWORD result = 0;
	int i;
...
void CCheckSector::CalcEccQ(unsigned char *buffer)
{
	unsigned char *Q;
	int i,j;
...
void CCheckSector::CalcEccP(unsigned char *buffer)
{
	unsigned char *P;
	int i,j;
