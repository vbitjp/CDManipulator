...
static DWORD WINAPI CreateIsoThread(LPVOID Thread)
{
	...
	RetValue = ((CCreateThread*)Thread)->ThreadFunction();
...
void CCreateThread::StartThread(void)
{
	StopThread();
	m_StopFlag = false;
	m_hThread = CreateThread(NULL,0,CreateIsoThread,this,0,&m_ThreadID);
...
{
	CCreateProgressDialog *dlg = (CCreateProgressDialog*)m_ParentWnd;
	bool RetValue;
...
bool CCreateThread::CreateIso(void)
{
	...
	hFile = CreateFile(m_ImgPath,GENERIC_WRITE,0,NULL,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,NULL);
	...
	//   write main data
	PrevTrackType = 0;
	for(i=0;i<(DWORD)(m_TrackList->GetItemCount());i++){
		...
		if(TrackType == 0){	//   data track
			...
			if(tt == "Mastering"){
				while(iso.GetHeaderFrame(Buffer)){
					...
					WriteFile(hFile,Buffer,2352,&wrote,NULL);
					if(wrote != 2352){
