...
DWORD CReadThread::CompareData(void)
{
	...
	hFile = CreateFile(ImgFileName,GENERIC_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
	if(hFile == INVALID_HANDLE_VALUE){
		m_LogWnd->AddMessage(LOG_ERROR,MSG(157));
		return 0;
	}
	SetFilePointer(hFile,DataPos*2352,NULL,FILE_BEGIN);
	...
	for(lba = lbaStart;lba < lbaEnd;lba++){
		...
		ReadFile(hFile,FileBuffer,2352,&read,NULL);
