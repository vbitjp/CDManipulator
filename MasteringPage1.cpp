...
CMasteringPage1::CMasteringPage1(CWnd* pParent /*=NULL*/)
	: CDialog(CMasteringPage1::IDD, pParent)
{
}
...
void CMasteringPage1::OnLvnKeydownTracklist(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	if(pLVKeyDow->wVKey == VK_DELETE){
		((CMasteringFileDialog*)m_MainDialog)->OnEditDeletetrack();
	}
}
...
void CMasteringPage1::OnNMRclickTracklist(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	GetCursorPos(&pos);
	m_TrackMenu.TrackPopupMenu(TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON,pos.x,pos.y,m_MainDialog,NULL);
...
void CMasteringPage1::InsertIsoTrack(LPCSTR FileName)
{
	...
	for(id = 1;id < m_TrackList.GetItemCount();id++){
		CString cs;
		cs.Format("%d",id+1);
		m_TrackList.SetItemText(id,0,(LPCSTR)cs);
	}
}
...
void CMasteringPage1::InsertWaveAudioTrack(void)
{
	...
	if(dlg.DoModal() == IDOK){
		...
		while(pos){
			...
			PathName = (LPCTSTR)dlg.GetNextPathName(pos);
			id = m_TrackList.InsertItem(m_TrackList.GetItemCount(),(LPCSTR)cs);
...
int CMasteringPage1::CheckIsoType(LPCSTR FileName)
{
	...
	if(hFile != INVALID_HANDLE_VALUE){
		ReadFile(hFile,Buffer,16,&read,NULL);
		...
		if(memcmp(Buffer,SyncHeader,12) == 0){
			...
			SetFilePointer(hFile,2048*16,0,FILE_BEGIN);
			ReadFile(hFile,Buffer,16,&read,NULL);
			if(Buffer[0] == 1 && Buffer[1] == 'C' && Buffer[2] == 'D' && Buffer[3] == '0' && Buffer[4] == '0' && Buffer[5] == '1' && Buffer[6] == 1){
				//   mode1/2048
				IsoType = 0;
			}
		}
