...
static DWORD WINAPI ReadThread(LPVOID Thread)
{
	...
	RetValue = ((CReadThread*)Thread)->ThreadFunction();
...
void CReadThread::StartThread(void)
{
	...
	m_hThread = CreateThread(NULL,0,ReadThread,this,0,&m_ThreadID);
...
DWORD CReadThread::GetTime(void)
{
	...
	return st.wMilliseconds + 1000 * (st.wSecond + 60 * (st.wMinute + 60 * (st.wHour + 24 * (st.wDay + 365 * st.wYear))));
}
