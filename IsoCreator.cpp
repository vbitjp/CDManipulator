...
void CIsoCreator::CreatePVD(BYTE *pvd)
{
	...
	sprintf((char*)(pvd+0x28),"%-30s",(LPCSTR)m_VolumeLabel);
...
void CIsoCreator::CreateSVD(BYTE *pvd)
{
	...
	memset(pvd+0x008,0x20,0x20);
	{
		...
		cs.Format("%-32s",(LPCSTR)m_VolumeLabel);
		ws = cs;
		ByteSwapCopy(pvd+0x28,(LPCWSTR)ws,32);
	}
	*((DWORD*)(pvd+0x50)) = m_ImageSize;
	*((DWORD*)(pvd+0x54)) = LeToBe(m_ImageSize);
...
{
	Buffer[0] = (BYTE)(Time.wYear - 1900);
	Buffer[1] = (BYTE)Time.wMonth;
	Buffer[2] = (BYTE)Time.wDay;
	Buffer[3] = (BYTE)Time.wHour;
	Buffer[4] = (BYTE)Time.wMinute;
	Buffer[5] = (BYTE)Time.wSecond;
	Buffer[6] = 0;
}
...
void CIsoCreator::CreatePathTable(int Type)
{
	...
	for(LeBe = 0;LeBe < 2;LeBe++){
		...
		while(p != NULL){
			...
			if(LeBe){
				*((WORD*)(Buffer + 6)) = LeToBeShort((WORD)p->m_ParentNo);
			} else {
				*((WORD*)(Buffer + 6)) = (WORD)p->m_ParentNo;
			}
			if(Type == 0){
				memcpy(Buffer+8,(LPCSTR)(p->m_Dir->m_ImageFileNameShort),p->m_Dir->m_ImageFileNameShort.GetLength());
			} else {
				CStringW cw;

				cw = p->m_Dir->m_ImageFileName;
				ByteSwapCopy(Buffer+8,(LPCWSTR)(cw),cw.GetLength()*2);
			}
...
CFileList * CIsoCreator::AddNode(CFileList * RootNode,CFileList * NewNode)
{
	...
	if(RootNode == NULL){
		return p;
	} else if(p->m_Dir->m_ImageFileName < RootNode->m_Dir->m_ImageFileName){
		p->SetNext(RootNode);
		return p;
	} else {
		q = RootNode;
		while(q != NULL){
			if(q->GetNext() == NULL){
				q->SetNext(p);
				break;
			} else if(p->m_Dir->m_ImageFileName < q->GetNext()->m_Dir->m_ImageFileName){
				p->SetNext(q->GetNext());
				q->SetNext(p);
				break;
			}
...
void CIsoCreator::CreateDirRecord(CFileList * List,int Type,DWORD Current,DWORD Parent)
{
	...
	*((DWORD*)(Buffer + 14)) = LeToBe(ParentSize);
	...
	while(List != NULL){
		...
		*((WORD*)(Buffer + 28)) = 1;
		*((WORD*)(Buffer + 30)) = LeToBeShort(1);
		Buffer[32] = (BYTE)length;
		if(Type == 0){
			CString cs;
			cs = List->m_Dir->m_ImageFileNameShort + ";1";
			memcpy(Buffer+33,(LPCSTR)cs,length);
		} else {
			CStringW cw;
			cw = List->m_Dir->m_ImageFileName + ";1";
			ByteSwapCopy(Buffer+33,(LPCWSTR)cw,length);
		}
...
{
	CloseReadFile();
	if(m_ReadingFile == NULL){
		return false;
	}
	m_hFile = CreateFile(m_ReadingFile->m_Dir->m_RealFileName,GENERIC_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
	if(m_hFile == INVALID_HANDLE_VALUE){
		return false;
	}
...
bool CIsoCreator::GetFrame(BYTE * Buffer)
{
	...
	ReadFile(m_hFile,m_Buffer,2048,&read,NULL);
	if(read == 0){
		if(OpenReadFile()){
			ReadFile(m_hFile,m_Buffer,2048,&read,NULL);
		} else {
