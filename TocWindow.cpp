...
//		msf = Toc->m_Track[Track].m_MSF.GetByLBA();
		cs.Format("%02d:%02d.%02d",Toc->m_Track[Track].m_MSF.Minute,Toc->m_Track[Track].m_MSF.Second,Toc->m_Track[Track].m_MSF.Frame);
...
{
	// TODO : ここにコマンド ハンドラ コードを追加します。
	CFileDialog dlg(FALSE,".txt",NULL,OFN_HIDEREADONLY |OFN_OVERWRITEPROMPT,STR(13));
...
{
	// TODO : ここにコマンド ハンドラ コードを追加します。
	CFileDialog dlg(FALSE,".html",NULL,OFN_HIDEREADONLY |OFN_OVERWRITEPROMPT,STR(14));
	...
	if(dlg.DoModal() == IDOK){
		...
		fprintf(fp,"<tr><td>トラック</td><td>種類</td><td>開始位置</td><td>トラック長</td><td>ギャップ</td><td>サイズ</td><td>デジタルコピー</td><td>エンファシス</td></tr>\n");
		for(i=0;i<m_View.GetItemCount();i++){
			TrackType	= m_View.GetItemText(i,1);
			Start		= m_View.GetItemText(i,2);
			Length		= m_View.GetItemText(i,3);
			...
			DigitalCopy = m_View.GetItemText(i,6);
			Emphasis	= m_View.GetItemText(i,7);
			fprintf(fp,"<tr><td>%d</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>\n",
				i,TrackType,Start,Length,Gap,Size,DigitalCopy,Emphasis);
		}
		fprintf(fp,"</body>\n</html>\n");
		fclose(fp);
...
{
	// TODO : ここにコマンド ハンドラ コードを追加します。
	CFileDialog dlg(FALSE,".csv",NULL,OFN_HIDEREADONLY |OFN_OVERWRITEPROMPT,STR(15));
...
{
	// TODO : ここにコマンド ハンドラ コードを追加します。
	CFileDialog dlg(FALSE,".cue",NULL,OFN_HIDEREADONLY |OFN_OVERWRITEPROMPT,MSG(22));
...
void CTocWindow::SetLanguage(void)
{
	...
	for(i=0;i<5;i++){
		GetMenu()->ModifyMenu(MenuString[i][0],MF_BYCOMMAND | MF_STRING,MenuString[i][0],theSetting.m_Lang.m_Str[LP_TOCMENU + MenuString[i][1]]);
	}
	SetWindowText(STR(0));

	if(m_View.GetHeaderCtrl() != NULL){
		HDITEM hi;
		int i;
		char Buffer[100];
