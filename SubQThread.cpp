...
static DWORD WINAPI SubQThread(LPVOID Thread)
{
	...
	RetValue = ((CSubQThread*)Thread)->ThreadFunction();
...
{
	StopThread();
	m_hThread = CreateThread(NULL,0,SubQThread,this,0,&m_ThreadID);
...
{
	CSubQProgressDialog *Dlg = (CSubQProgressDialog *)m_ParentWnd;
	m_Error = false;
...
void CSubQThread::EntryToc(BYTE Track, BYTE Idx, BYTE Minute, BYTE Second, BYTE Frame, BYTE TrackType, BYTE CtlAdr, BYTE SessionNo)
{
	CSubQProgressDialog *Dlg = (CSubQProgressDialog *)m_ParentWnd;
	CString cs;
