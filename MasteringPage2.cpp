...
CMasteringPage2::CMasteringPage2(CWnd* pParent /*=NULL*/)
	: CDialog(CMasteringPage2::IDD, pParent)
	, m_VolumeLabel(_T(""))
{
}
...
BOOL CMasteringPage2::OnInitDialog()
{
	...
	m_TreeImageNormal.Create(IDB_FILE_TREE,12,2,(COLORREF)0xffffff);
	...
	m_ListImage.Create(IDB_FILE_LIST,12,2,(COLORREF)0xffffff);
...
void CMasteringPage2::OnLvnKeydownList(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	if(pLVKeyDow->wVKey == VK_DELETE){
		((CMasteringFileDialog*)m_MainDialog)->OnEditDeletefolder();
	} else if(pLVKeyDow->wVKey == VK_F2){
		((CMasteringFileDialog*)m_MainDialog)->OnEditLabel();
	}
}
...
void CMasteringPage2::OnNMRclickList(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	GetCursorPos(&pos);
	m_Menu.TrackPopupMenu(TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON,pos.x,pos.y,m_MainDialog,NULL);
...
bool CMasteringPage2::AddFile(LPCSTR FileName)
{
	...
	if(p->SearchName(sp) == NULL && p->SearchFile(FileName) == NULL){
		...
		hFile = CreateFile(FileName,0,FILE_SHARE_READ|FILE_SHARE_WRITE,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
		...
		((CMasteringPage1*)m_Page1)->InsertMode1MasteringTrack();
		return true;
	}
	return false;
}
...
CDirStructure * CMasteringPage2::AddFolder(LPCSTR FolderName)
{
	...
	if(p->SearchName(FolderName) == NULL){
		HTREEITEM NewNode;
...
void CMasteringPage2::OnDropFiles(HDROP hDropInfo)
{
	...
	UpdateData(TRUE);
	max = DragQueryFile(hDropInfo,0xffffffff,NULL,0);
	for(i=0;i<max;i++){
	...
	((CMasteringFileDialog*)m_MainDialog)->CalcSize();
}
