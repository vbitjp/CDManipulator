...
static DWORD WINAPI WriteThread(LPVOID Thread)
{
	...
	RetValue = ((CWriteThread*)Thread)->ThreadFunction();
...
void CWriteThread::StartThread(void)
{
	StopThread();
	m_StopFlag = false;
	m_hThread = CreateThread(NULL,0,WriteThread,this,0,&m_ThreadID);
...
DWORD CWriteThread::ThreadFunction(void)
{
	...
	CWriteProgressDialog *Dlg = (CWriteProgressDialog*)m_ParentWnd;

	Dlg->m_Progress.SetPos(0);
	Dlg->m_Percent = "0%";
	PostMessage(Dlg->m_hWnd,WM_COMMAND,ID_UPDATE_DIALOG,0);

	if(m_Dir != NULL && m_List != NULL){
		RetValue = Mastering();
	} else {
...
DWORD CWriteThread::WriteImage(void)
{
	...
	if(m_ModeMS){
		...
		if(m_CueFileName.Right(3).MakeLower() == "iso"){
			...
			hFile = CreateFile(m_CueFileName,GENERIC_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
			if(hFile == INVALID_HANDLE_VALUE){
				return 0;
			}
			FileSize = GetFileSize(hFile,NULL);
			ReadFile(hFile,&ReadBuffer,16,&ReadSize,NULL);
			CloseHandle(hFile);
	...
	while(!m_CD->CheckDisc()){
		if(MessageBox(NULL,MSG(30),CONF_MSG,MB_YESNO) == IDNO){
...
DWORD CWriteThread::WriteImageSubMS(void)
{
	...
	ret = 1;
	hImg = CreateFile(m_SubMS.m_ImgFileName,GENERIC_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
	if(!hImg){
	...
	size = GetFileSize(hImg,NULL) / 2352;
	hSub = CreateFile(m_SubMS.m_SubFileName,GENERIC_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
	if(!hSub){
		Dlg->m_Message = MSG(43);
		m_LogWnd->AddMessage(LOG_ERROR,MSG(43));
		PostMessage(Dlg->m_hWnd,WM_COMMAND,ID_UPDATE_DIALOG,0);
	...
	if(m_SubMS.m_PreFileName != ""){
		hPre = CreateFile(m_SubMS.m_PreFileName,GENERIC_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
	}
	...
	for(session = 0;session < m_SubMS.GetSessionCount();session++){
		...
		for(;lba != m_SubMS.m_MainDataLBA[session];lba++){
			...
			if(session == 0 && hPre != INVALID_HANDLE_VALUE){
				ReadFile(hPre,Buffer,2352,&read,NULL);
			} else {
				m_SubMS.CreateZeroData(Buffer,lba);
			}
			if(m_CD->GetWritingMode() != WRITEMODE_RAW_16){
		...
		for(;;lba++){
			...
			ReadFile(hSub,SubBuffer,96,&read,NULL);
			ReadFile(hImg,Buffer,2352,&read,NULL);
			if(m_CD->GetWritingMode() != WRITEMODE_RAW_16){
...
DWORD CWriteThread::WriteImageSubSS(void)
{
	...
	PostMessage(Dlg->m_hWnd,WM_COMMAND,ID_UPDATE_DIALOG,0);
	if(m_CD->GetImageFileName() == NULL || *(m_CD->GetImageFileName()) == '\0'){
		hFile = CreateFile(m_CueFileName,GENERIC_READ,FILE_SHARE_READ|FILE_SHARE_WRITE,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
	} else {
		hFile = CreateFile(m_CD->GetImageFileName(),GENERIC_READ,FILE_SHARE_READ|FILE_SHARE_WRITE,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
	}
	if(hFile == INVALID_HANDLE_VALUE){
		Dlg->m_Message = MSG(43);
		m_LogWnd->AddMessage(LOG_ERROR,MSG(43));
	...
	for(i=0;i<m_CD->GetImageFrames();i++){
		ReadFile(hFile,Buffer,2352,&read,NULL);
		if(m_StopFlag){
			return 0;
		}
...
DWORD CWriteThread::MasteringSub(void)
{
	...
	PostMessage(Dlg->m_hWnd,WM_COMMAND,ID_UPDATE_DIALOG,0);
	PrevTrackType = 0;
	for(i=0;i<(DWORD)(m_List->GetItemCount());i++){
		...
		if(TrackType == 0){	//   data track
			...
			if(tt == "Mastering"){
				...
				hFileRead = CreateFile(FileName,GENERIC_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
				...
				while(1){
					...
					ReadFile(hFileRead,Buffer+16,2048,&read,NULL);
					if(read < 2048){
						break;
					}
					msf = lba;
				...
				while(1){
					...
					ReadFile(hFileRead,Buffer,2352,&read,NULL);
					if(read < 2352){
						break;
					}
					if(!m_CD->WriteRaw(Buffer)){
	...
	for(i=0;i<m_List->GetItemCount();i++){
		...
		if(TrackType == 0){	//   data track
			...
			if(tt == "Mastering"){
				...
				hFile = CreateFile(cs,0,FILE_SHARE_READ,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
				FileSize = GetFileSize(hFile,NULL);
				CloseHandle(hFile);
				if(tt == "MODE1/2048"){
					cs.Format("  TRACK 01 MODE1/2352\n");
			...
			if(SkipAudioHeader(hFile)){
				DWORD np,ep,size;
				np = SetFilePointer(hFile,0,NULL,FILE_CURRENT);
				ep = SetFilePointer(hFile,0,NULL,FILE_END);
				size = ep - np;
...
bool CWriteThread::SkipAudioHeader(HANDLE hFile)
{
	...
	ReadFile(hFile,&ch,sizeof(ch),&read,NULL);
	if(ch.chunk_id != 'FFIR'){
		return false;
	}
	ReadFile(hFile,&format,sizeof(format),&read,NULL);
	if(format != 'EVAW'){
		return false;
	}

	while(1){
		if(!ReadFile(hFile,&ch,sizeof(ch),&read,NULL)){
			return false;
		}
		if(ch.chunk_id == 'atad'){
			break;
		}
		SetFilePointer(hFile,ch.chunk_size,NULL,FILE_CURRENT);
	}
