// MasteringFileDialog.cpp : 実装ファイル
//
...
// CMasteringFileDialog メッセージ ハンドラ
...
void CMasteringFileDialog::OnTrackIso()
{
	// TODO : ここにコマンド ハンドラ コードを追加します。
	if(m_Page1.GetStyle() & 0x10000000){
		CFileDialog dlg(TRUE,NULL,NULL,OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_ALLOWMULTISELECT,MSG(90));
		char buf[2048];
		char *p;
...
void CMasteringFileDialog::OnBnClickedCreateIso()
{
	...
	if(Dlg.DoModal() == IDOK){
		...
		CreateDlg.CreateIso(Dlg.GetPathName(),(LPCSTR)(*m_VolumeLabel),m_TrackList,&m_Dir,m_LogWnd);
	}
	if(theSetting.m_Mastering_AlwaysOnTop){
		SetWindowPos(&wndTopMost,0,0,0,0,SWP_NOMOVE | SWP_NOSIZE);
	}
...
void CMasteringFileDialog::OnBnClickedWriting()
{
	...
	if(Dlg.DoModal() == IDOK){
		...
		WriteDlg.Mastering(&m_Dir,(LPCSTR)(*m_VolumeLabel),m_TrackList,m_CD,m_LogWnd);
	}
	if(theSetting.m_Mastering_AlwaysOnTop){
		SetWindowPos(&wndTopMost,0,0,0,0,SWP_NOMOVE | SWP_NOSIZE);
	}
...
void CMasteringFileDialog::CalcSize(void)
{
	...
	for(i=0;i<m_TrackList->GetItemCount();i++){
		if(m_TrackList->GetItemData(i) == 0){
			...
			cs = m_TrackList->GetItemText(i,2);
			hFile = CreateFile(cs,0,FILE_SHARE_READ,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
			size += (GetFileSize(hFile,NULL) + 2352 - 45) / 2352;
			CloseHandle(hFile);
		}
	}
	msf = size;
	...
	if(m_LeadOutPos == 0){
		...
		MSFAddress lo;
		lo = m_LeadOutPos - 150;
		m_Size.Format("%02d:%02d:%02d / %02d:%02d:%02d",msf.Minute,msf.Second,msf.Frame,lo.Minute,lo.Second,lo.Frame);
	}
	UpdateData(FALSE);
}
...
void CMasteringFileDialog::SetLanguage(void)
{
	...
	for(i=0;i<13;i++){
		GetMenu()->ModifyMenu(MenuString[i][0],MF_BYCOMMAND | MF_STRING,MenuString[i][0],theSetting.m_Lang.m_Str[LP_MASTERINGMENU + MenuString[i][1]]);
	}
	SetWindowText(STR(0));
	for(i=0;i<5;i++){
...
{
	// TODO : ここにコントロール通知ハンドラ コードを追加します。
	ShellExecute(m_hWnd, "explore", NULL, NULL, NULL, SW_SHOWNORMAL);
}
